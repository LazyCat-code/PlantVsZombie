...
public class Zombies0 : ZombieEnemy
{
    ...
    public GameObject Zombieponcho;  // 帽子
    public int PonchoHealht; // 帽子血量

    public int HalfHealht;  // 僵尸半血
    ...
    protected override void Start()
    {
        ...
        health += PonchoHealht;  // 加上帽子血量

        HalfHealht = health / 2; // 僵尸半血
    }
    ...
    {
        // 当小于自己血量一半时候帽子失效
        if (health < HalfHealht)
        {
            Zombieponcho.SetActive(false);
        }
        // 死亡动画
        if (health < 0)
        {
            isWalk = false;
            animator.SetTrigger("Die0");
