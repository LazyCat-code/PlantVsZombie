    ...
    public void OnEndDrag(BaseEventData data)
    {
        ...
        foreach(Collider2D c in col)
        {
            ...
            if(c.tag == "Land" && c.transform.childCount == 0)
            {
                ...
                GameMusicSound.instance.PlantsZzBGM();
                curGameObject.GetComponent<Plants>().SetPlantStart(); // 调用植物父类启用方法
                curGameObject = null;
                // 消耗阳光
                SunUI.CurrentSuning -= useSun;
                if (!dateTool.PeashooterToos.PlantCardTime)
                {
                    timer = 0; // 重置计时器
                }
