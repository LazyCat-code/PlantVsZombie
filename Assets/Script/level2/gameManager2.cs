...
public class gameManager2 : MonoBehaviour
{
    ...
    IEnumerator DalayCreateZombie()
    {
        ...
        while (true)
        {
            ...
            if (Timer < LevelTime)
            {
                ...
                GameObject zombie = Instantiate(zombiePrefab[Random.Range(0, 5)]); //获取僵尸对象并且重置属性 并且随机生成僵尸种类
            ...
            if (Timer / LevelTime > LevelUInum && Zombielevelnum <= 4)
            {
                ...
                {
                    GameObject zombie = Instantiate(zombiePrefab[Random.Range(1, 5)]); //获取僵尸对象并且重置属性 并且随机生成僵尸种类
    ...
    IEnumerator GameOver()
    {
        ...
        SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex - 4);  // 返回开始界面
    }
